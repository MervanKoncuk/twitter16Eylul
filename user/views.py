    ...
    if request.method == 'POST':
        if 'register' in request.POST:
            ...
            if password1 == password2:
                if User.objects.filter(username = username).exists():
                ...
                elif username in password1: # mervan - mervan123
                    messages.error(request, 'Şifreniz ile kullanıcı adınınz benzer olmamalıdır')
                    ...
                    if gender == 'female':
                        profile.image = 'profiles/default-female.jpeg'
                    else:
                        profile.image = 'profiles/default-male.jpeg'
                    profile.save()
                    login(request, user)
    ...
    if request.method == 'POST':
        if 'change' in request.POST:
            ...
            if user is not None:
                ...
                hata = cache.get('deneme')
                hata += 1
                cache.set('deneme', hata, 1000000000)
                if hata == 3:
                    logout(request)
                    messages.error(request, '3ten fazla deneme yaptığınız hesabınıza belli bir süreliğine giriş yapamazsınız')
                    return redirect('login')
        ...
        if profileForm.is_valid():
            profileForm.save()
            if newUsername and newEmail: # "" = False  "abc" = True
